...
def on_press(key):
    ...
    try:
        ...
        if key_on_press == 'x' and place:
            if not lock:
                action['equip'] = place_l[place_i]
            place = False
            last_ = 'place'
            print(place_l[place_i], obs['equipped_items']['mainhand']['type'])
...
place = True
place_l = ["carrot",'fence', "fence_gate", 'snowball', 'wheat_seeds', 'wheat']
place_i = 0
...
while (True):
    ...
    if pasue:
        ...
        # action['nearbySmelt'] = 'iron_ingot'a
        # x['jump'] = 0
        # x['forward'] = 1
        # x['camera'] = [0,15]
        #
