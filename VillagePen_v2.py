...
# main_all_i = np.flip(np.arange(0, int(max_his / 2), 1))
...
def train():
    ...
    for trajectory_name in trajectory_names:
        ...
        last_non_action = 'none'
        last_seri_action_index = [-1 for _ in range(int(max_his/2))]
        last_action_indes = 0
        ...
        for obs, action, r, _, _ in trajectory:
            ...
            old_his = copy.deepcopy(last_seri_action_index)
            if old_his[-1] != len(list_h):
                old_his.append(len(list_h))
                del old_his[0]
            list_h.append(old_his)

            if action['use'] == 1:
                last_seri_action_index.append(len(list_h))
                del last_seri_action_index[0]
                last_action_indes = 0
                last_non_action = 'use'

            elif action['attack'] == 1:
                if last_non_action != 'attack' or last_action_indes >= 100:
                    last_action_indes = 0
                    last_non_action = 'attack'
                    last_seri_action_index.append(len(list_h))
                    del last_seri_action_index[0]

            elif action['jump'] == 1:
                if last_non_action != 'jump' or last_action_indes >= 100:
                    last_action_indes = 0
                    last_non_action = 'jump'
                    last_seri_action_index.append(len(list_h))
                    del last_seri_action_index[0]

            elif action['equip'] != 'none':
                last_action_indes = 0
                last_non_action = 'equip'
                last_seri_action_index.append(len(list_h))
                del last_seri_action_index[0]
    ...
    for index__ in range(10):
        ...
        for batch_i in range(0, num_samples, BATCH_SIZE):
            ...
            for j in range(len(batch_indices)):
                ...
                c_h = c_history[index]
                #
                # tmp_list = list_h[index]
                # tmp_list = np.concatenate((main_all_i, tmp_list), axis=0)
                counter_i = 0
                for b in main_all_i:
                    n_index = index - b
                    if n_index >= 0:
...
for episode in range(20):
    ...
    for i in range(18000):
        ...
        old_his = copy.deepcopy(last_seri_action_index)
        if old_his[-1] != len(stack_all):
            old_his.append(len(stack_all))
            del old_his[0]
        tmp_list = np.concatenate((main_all_i, old_his), axis=0)
        ...
        if new_a['use'] == 1:
            ...
            #     if done:
            #         break
            last_seri_action_index.append(len(stack_all))
            del last_seri_action_index[0]
            last_action_indes = 0
            last_non_action = 'use'
