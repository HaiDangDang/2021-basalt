...
with open('home_action.pkl', 'rb') as f:
    home_action = pickle.load(f)
...
ENV_RUN = 'MineRLBasaltBuildVillageHouse-v0'
...
def process_inventory(obs, angle_1, angle_2):
    ...
    if obs['equipped_items']['mainhand']['type'] in ['acacia_door']:
        data[0] = 1
    if obs['equipped_items']['mainhand']['type'] in ['acacia_fence']:
        data[1] = 1
    if obs['equipped_items']['mainhand']['type'] in ['cactus']:
        data[2] = 1
    if obs['equipped_items']['mainhand']['type'] in ['cobblestone']:
        data[3] = 1
    if obs['equipped_items']['mainhand']['type'] in ['dirt']:
        data[4] = 1
    if obs['equipped_items']['mainhand']['type'] in ['fence']:
        data[5] = 1
    if obs['equipped_items']['mainhand']['type'] in ['flower_pot']:
        data[6] = 1
    if obs['equipped_items']['mainhand']['type'] in ['glass']:
        data[7] = 1
    if obs['equipped_items']['mainhand']['type'] in ['ladder']:
        data[8] = 1
    if obs['equipped_items']['mainhand']['type'] in ['log#0']:
        data[9] = 1
    if obs['equipped_items']['mainhand']['type'] in ['log#1']:
        data[10] = 1
    if obs['equipped_items']['mainhand']['type'] in ['log2#0']:
        data[11] = 1
    if obs['equipped_items']['mainhand']['type'] in ['planks#0']:
        data[12] = 1
    if obs['equipped_items']['mainhand']['type'] in ['planks#1']:
        data[13] = 1
    if obs['equipped_items']['mainhand']['type'] in ['planks#4']:
        data[14] = 1
    if obs['equipped_items']['mainhand']['type'] in ['red_flower']:
        data[15] = 1
    if obs['equipped_items']['mainhand']['type'] in ['sand']:
        data[16] = 1
    if obs['equipped_items']['mainhand']['type'] in ['sandstone#0']:
        data[17] = 1
    if obs['equipped_items']['mainhand']['type'] in ['sandstone#2']:
        data[18] = 1
    if obs['equipped_items']['mainhand']['type'] in ['sandstone_stairs']:
        data[19] = 1
    if obs['equipped_items']['mainhand']['type'] in ['snowball']:
        data[20] = 1
    if obs['equipped_items']['mainhand']['type'] in ['spruce_door']:
        data[21] = 1
    if obs['equipped_items']['mainhand']['type'] in ['spruce_fence']:
        data[22] = 1
    if obs['equipped_items']['mainhand']['type'] in ['stone_axe']:
        data[23] = 1
    if obs['equipped_items']['mainhand']['type'] in ['stone_pickaxe']:
        data[24] = 1
    if obs['equipped_items']['mainhand']['type'] in ['stone_stairs']:
        data[25] = 1
    if obs['equipped_items']['mainhand']['type'] in ['torch']:
        data[26] = 1
    if obs['equipped_items']['mainhand']['type'] in ['wooden_door']:
        data[27] = 1
    if obs['equipped_items']['mainhand']['type'] in ['wooden_pressure_plate']:
        data[28] = 1
    if obs['equipped_items']['mainhand']['type'] in ['other', 'none', 'air']:
        data[29] = 1

    data[30] = np.clip(obs['inventory']['acacia_door'] / 64, 0, 1)
    data[31] = np.clip(obs['inventory']['acacia_fence'] / 64, 0, 1)
    data[32] = np.clip(obs['inventory']['cactus'] / 3, 0, 1)
    data[33] = np.clip(obs['inventory']['cobblestone'] / 64, 0, 1)
    data[34] = np.clip(obs['inventory']['dirt'] / 64, 0, 1)
    data[35] = np.clip(obs['inventory']['fence'] / 64, 0, 1)
    data[36] = np.clip(obs['inventory']['flower_pot'] / 3, 0, 1)
    data[37] = np.clip(obs['inventory']['glass'] / 64, 0, 1)
    data[38] = np.clip(obs['inventory']['ladder'] / 64, 0, 1)
    data[39] = np.clip(obs['inventory']['log#0'] / 64, 0, 1)

    data[40] = np.clip(obs['inventory']['log#1'] / 64, 0, 1)
    data[41] = np.clip(obs['inventory']['log2#0'] / 64, 0, 1)
    data[42] = np.clip(obs['inventory']['planks#0'] / 64, 0, 1)
    data[43] = np.clip(obs['inventory']['planks#1'] / 64, 0, 1)
    data[44] = np.clip(obs['inventory']['planks#4'] / 64, 0, 1)
    data[45] = np.clip(obs['inventory']['red_flower'] / 3, 0, 1)
    data[46] = np.clip(obs['inventory']['sand'] / 64, 0, 1)
    data[47] = np.clip(obs['inventory']['sandstone#0'] / 64, 0, 1)
    data[48] = np.clip(obs['inventory']['sandstone#2'] / 64, 0, 1)
    data[49] = np.clip(obs['inventory']['sandstone_stairs'] / 64, 0, 1)

    data[50] = np.clip(obs['inventory']['snowball'] / 1, 0, 1)
    data[51] = np.clip(obs['inventory']['spruce_door'] / 64, 0, 1)
    data[52] = np.clip(obs['inventory']['spruce_fence'] / 64, 0, 1)

    data[53] = np.clip(obs['inventory']['stone_stairs'] / 64, 0, 1)
    data[54] = np.clip(obs['inventory']['torch'] / 64, 0, 1)
    data[55] = np.clip(obs['inventory']['wooden_door'] / 64, 0, 1)
    data[56] = np.clip(obs['inventory']['wooden_pressure_plate'] / 64, 0, 1)


    data[57] = np.clip((angle_1 + 90) / 180, 0, 1)

    if angle_2 > 180:
        angle_2 = 360 - angle_2
        data[59] = 1
    data[58] = np.clip(angle_2/ 180, 0, 1)

    return data

range_angle = np.array([-30, -15, -10, -5, 0, 5, 10, 15, 30])
range_angle_2 = np.array([-30, -15, -10, -5, 0, 5, 10, 15, 30])


list_key = [ np.array(list(home_action.keys())), ['0', 'j_f', 'f', 'b', 'l', 'r'],range_angle_2, range_angle ]
...
def dataset_action_batch_to_actions(actions):
    ...
    if len(np.where(np.array(list_key[0]) == ks[0])[0]) == 0:
        print(ks[0])
        ks[0] = '0'
...
def train():
    ...
    th.save(network.state_dict(), 'home/a_1.pth')
    th.save(network_1.state_dict(), 'home/a_2.pth')
    th.save(network_2.state_dict(), 'home/a_3.pth')
    th.save(network_3.state_dict(), 'home/a_4.pth')
    del data
...
network.load_state_dict(th.load('home/a_1.pth'))

network_1 = NatureCNN((max_shape, 64, 64), len(list_key[1])).cuda()
network_1.load_state_dict(th.load('home/a_2.pth'))

network_2 = NatureCNN((max_shape, 64, 64), len(list_key[2])).cuda()
network_2.load_state_dict(th.load('home/a_3.pth'))

network_3 = NatureCNN((max_shape, 64, 64), len(list_key[3])).cuda()
network_3.load_state_dict(th.load('home/a_4.pth'))
...
# with open('home_action.pkl', 'wb') as f:
#     pickle.dump(home_action, f)
